({
	...
	getAccountInfo : function(component, event) {
        var myAction = component.get("c.getAccountInfo");
        
        //傳入event id。
        myAction.setParams({
            accountId : event.getParam("PassAccount")
        });
        ...
        myAction.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                var controllerValueKey = component.get('v.SelectedAccount.CompanyCity__c');
                var depedentValue = component.get('v.SelectedAccount.CompanyDistrict__c');
                 component.find('conCountry').set("v.value", controllerValueKey);
                // get the map values
                
                if(controllerValueKey === undefined)
                {
                    controllerValueKey = '';
                }
                ...
                if (controllerValueKey != '') {
                    ...
                    component.find('conState').set("v.value", depedentValue);
