({
	initial : function(component, event, helper) {
		...
		//傳入event id。
		myAction.setParams({
			inputKeywordId : component.get("v.LineKeyWordID")
		});
        ...
        myAction.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
				...
				component.set('v.TheKeyWord', returnRecord);
	...
	saveMessage : function(component, event, helper) {
		var myContent = component.get('v.TheKeyWord');
		
		var contentList = component.get('v.TheMessageList2');
		var JSONString = "{"
		JSONString += '"theKeyWord":';
        ...
        myAction.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
				var returnRecord = response.getReturnValue();
				component.set("v.LineKeyWordID", returnRecord);
				component.set("v.TheKeyWord.Id", returnRecord);
				var toastEvent = $A.get("e.force:showToast");
					toastEvent.setParams({
						"type":"success",
						"title": "Success!",
	...
	checkMyMessage : function(component, event) {
		var errorMSG = "";
		var contentList = component.get('v.TheMessageList2');
