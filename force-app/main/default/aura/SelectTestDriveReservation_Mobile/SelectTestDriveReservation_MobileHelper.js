({
        ...
        myAction.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                var opts1 = [
                    ...
                    { class: "optionClass", label: "C", value: "C"},
                    { class: "optionClass", label: "E", value: "E"},
                    { class: "optionClass", label: "F", value: "F"}
                ];
                component.find("customerRating").set("v.options", opts1);
                
                var allList = response.getReturnValue();
                var opts2 = [];
                opts2.push({ class: "optionClass", label: "--無--", value: ""});
                for(var i=0; i<allList.length; i++)
                {
                    opts2.push({ class: "optionClass", label: allList[i], value: allList[i]});
                }
        		component.find("nextSubject").set("v.options", opts2);
            } else if (state === "INCOMPLETE") {
                // do something
            } else if (state === "ERROR") {
                var errors = response.getError();
    ...
    saveTheTDR: function(component, event) {
        ...
        var myMessage = "";
        myWorkLog.CustomerRating__c = component.find("customerRating").get("v.value");
        var myNextSubject = component.find("nextSubject").get("v.value");
